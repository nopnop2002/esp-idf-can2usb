...
void twai_task(void *pvParameters)
{
	...
	FRAME_t frameBuf;
	while (1) {
		//A watchdog occurs when the twai_receive() timeout is set to [pdMS_TO_TICKS (1)] on the ESP32-S2.
		//With the same code, the watchdog does not occur on the ESP32.
		...
		if (ret == ESP_OK) {
			...
			for(int index=0;index<npublish;index++) {
				...
				if (publish[index].canid == rx_msg.identifier) {
					...
					if (xQueueSend(xQueue_usb, &frameBuf, portMAX_DELAY) != pdPASS) {
